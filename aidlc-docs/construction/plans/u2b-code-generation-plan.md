...
- [x] `packages/rfd-runner/pyproject.toml` — `requires-python = ">=3.9,<3.10"` (matches the container
...
- [x] `packages/rfd-runner/src/rfd_runner/` package skeleton + `py.typed` marker
- [x] `packages/rfd-runner/tests/` directory
- [x] Confirm root `pyproject.toml`'s `members = ["packages/*"]` glob already picks this up (no edit
...
- [x] `config.py` — `RunnerConfig` (env-var-driven, `from_env()` pattern matching `rfd_core.PathLayout`):
...
- [x] `errors.py` — `RunnerError` base; `AnanasUnavailableError` (business-rules.md §3, carries the
...
- [x] `_colabdesign.py` — the bridge module (see Design Decision above): `pdb_to_string(path, chains)`,
...
- [x] `template.py` — `TemplateResolver.resolve_template(pdb, run_dir, *, fetch=None) -> Optional[Path]`
...
- [x] `symmetry_detector.py` — `SymmetryDetector.detect_symmetry(pdb_str, run_dir, *, ananas_bin=None,
...
- [x] `contig_normaliser.py` — `ContigNormaliser.normalise_contigs(spec, mode, parsed_pdb, copies) ->
...
- [x] `inference_executor.py` — `InferenceResult` dataclass (`exit_code`, `stderr_tail`);
...
- [x] `frame_publisher.py` — `FramePublisher` class exactly as specified in `component-methods.md` C-15
...
- [x] `progress_reporter.py` — `ProgressReporter` class per C-16 (`update_step`, `set_frame`,
      `set_stage`), writing `rfd_core.models.ProgressState` via `.save(run_dir)`
- [x] `pdb_postprocessor.py` — `PdbPostProcessor.fix_outputs(run_dir, name, num_designs, contigs) ->
...
- [x] `validation_executor.py` — `ValidationExecutor.run_validation(run_dir, name, normalised_contigs,
...
- [x] `result_packager.py` — `ResultPackager.stage_out(tmpdir, run_dir) -> None` (a no-op assertion:
...
- [x] `orchestrator.py` — `Stage(str, Enum)` (NOT `StrEnum` — Python 3.9, same fix as U2a's
...
- [x] `__main__.py` — `argparse` CLI: positional `run_dir`, `--stage {all,backbone,validate}` (default
...
- [x] `tests/test_config.py` — every `RunnerConfig` field's env-var override and documented default
- [x] `tests/test_template.py` — local path pass-through (no `fetch` call); 4-char code builds the
...
- [x] `tests/test_symmetry_detector.py` — missing/non-executable `ananas_bin` raises
...
- [x] `tests/test_contig_normaliser.py` — fixed/partial/free modes route to the correct bridge function
      via a fake `fix_contigs`/`fix_partial_contigs`; `copies > 1` replication matches
      `sum([contigs] * copies, [])` exactly
- [x] `tests/test_inference_executor.py` — against a fake `Popen`-shaped object and `tmp_path` dumps:
...
- [x] `tests/test_frame_publisher.py` — publishes only every `every_n` steps; `enabled=False` never
      writes; target path and atomicity (via `rfd_core.storage`) verified in `tmp_path`
- [x] `tests/test_progress_reporter.py` — `update_step`/`set_frame`/`set_stage` produce the correct
      `ProgressState` fields, real round-trip through `rfd_core` in `tmp_path`
- [x] `tests/test_pdb_postprocessor.py` — for `num_designs > 1`, all three paths per design constructed
      correctly and passed through a fake `fix_pdb`; file contents rewritten, not appended
- [x] `tests/test_validation_executor.py` — `val_argv` matches business-logic-model.md §2 field-for-field
      including optional-flag omission when unset; `cwd` defaults to `RunnerConfig.af_params_dir` and is
      overridable
- [x] `tests/test_result_packager.py` — real `zipfile` I/O in `tmp_path`: archive contains exactly the
      `{name}*` and `traj/{name}*` members, nothing else; `stage_out` is a no-op that does not raise
      when the invariant holds
- [x] `tests/test_orchestrator.py` — full control flow against `OrchestratorDeps` built entirely from
...
- [x] Confirm every row of business-logic-model.md §2's numbered flow and every rule in
      business-rules.md §1–§6 has a corresponding test — cross-check explicitly, not by inspection alone

### Step 5: Documentation Generation
- [x] `aidlc-docs/construction/u2b-runner/code/u2b-code-summary.md`

### Step 6: Configuration Artifact Update
- [x] Add `RFD_STEP_TIMEOUT_SECONDS` and `RFD_POLL_INTERVAL_MS` to `env.example` (currently only
...
- [x] `uv sync` at the workspace root (picks up `rfd-runner` via the existing `packages/*` glob)
- [x] `uv run --package rfd-runner pytest -v` — full suite, real Python 3.9.25, locally, now
- [x] `uv run --package rfd-runner python -c "import rfd_runner"` — proves the package imports cleanly
